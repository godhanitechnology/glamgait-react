...
//           sizeResponse,
//         ] = await Promise.all([
//           axiosInstance.get(`${ApiURL}/getsubcategory`),
//           axiosInstance.get(`${ApiURL}/getfabrics`),
//           axiosInstance.get(`${ApiURL}/getworks`),
//           axiosInstance.get(`${ApiURL}/getoccasions`),
//           axiosInstance.get(`${ApiURL}/getcolor`),
//           axiosInstance.get(`${ApiURL}/getsize`),
//         ]);
//         setSubCategories(subCategoryResponse.data.data || []);
//         setFabrics(fabricResponse.data.data || []);
//         setWorks(workResponse.data.data || []);
//         setOccasions(occasionResponse.data.data || []);
//         setColorsList(colorResponse.data.data || []);
//         setSizesList(sizeResponse.data.data || []);
//       } catch (error) {
//         console.error("Error fetching data:", error);
//         toast.error("Failed to fetch subcategories, colors, or sizes");
//       }
//     };
//     fetchData();
//   }, [token]);
...
//       setExistingSizes(product.productsizes || []);
//       setDeletedMediaIds([]);
//       setMediaPreviews([]);
//     } else {
//       setFormData({
...
//         fit: "",
//         is_expert_choice: false,
//         colors: [{ color_id: "", productimages: [] }], // Consistent
//         sizes: [{ size_id: "" }],
//       });
//       setExistingMedia([]);
//       setExistingSizes([]);
...
//     ]); // FIX: Use 'productimages' for existing (keep separate)
//     setDeletedMediaIds((prev) => [...prev, []]);
//   };

//   const removeColorField = (index) => {
//     // Add all remaining productimages to deleted (since removing whole color)
...
//     // FIX: No change needed here - backend handles full color removal anyway
//     // (deleted IDs for this index are sent, even if we filter; but since backend deletes all, it's ok)
...
//     // FIX: Simplified check - existingImages is already filtered to remaining (post-deletes)
//     // No need to subtract deletedMediaIds.length again
//     if (
...
//       toast.error("Please select a color for each color field");
//       setIsSubmitting(false);
//       return;
//     }
...
//       toast.error("Please select a size for each size field");
//       setIsSubmitting(false);
//       return;
//     }
...
//     // Append colors JSON (without productimages, as they are files)
//     data.append(
//       "colors",
//       JSON.stringify(
...
//     }
//     console.log(deletedMediaIds, "Deleted Media IDs");
...
//           error.response?.data?.errors?.media ||
//           "Failed to save product"
//       );
//     } finally {
//       setIsSubmitting(false);
  ...
  useEffect(() => {
    ...
    const fetchCategoryData = async () => {
      try {
        ...
        console.error(err);
        toast.error("Failed to fetch category related data");
      }
    };
    fetchCategoryData();
  }, [formData.cate_id]);
