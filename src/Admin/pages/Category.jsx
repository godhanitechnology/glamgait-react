        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody>
              {filteredCategories?.map((category) => (
                <tr key={category?.cate_id}>
                  <td className="px-6 py-4">{category?.cate_name}</td>
                  ...
                  <td className="px-6 py-4">
                    {category?.cate_chart ? (
                      <img
                        src={`${ApiURL}/assets/Category/${category?.cate_chart}`}
                        className="h-10 w-10 rounded object-cover"
                      />
                    ) : (
                      <span>No Chart</span>
                  ...
                  <td className="px-6 py-4 text-right">
                    <button
                      onClick={() => {
                        ...
                        setChartPreview(
                          category?.cate_chart
                            ? `${ApiURL}/assets/Category/Chart/${category?.cate_chart}`
                            : null
                        );

                        setMediaType(
                          category?.cate_image?.match(/\.(mp4)$/i)
                            ? "video"
                            : "image"
                        );
